 ...
 * This is the crucial step for the Gmail API.
...
function createRawEmail(to, subject, htmlContent) {
    const boundary = 'taskit-boundary';
    
    // Construct the full MIME message with necessary headers for HTML content
    const raw = [
        ...
        '',
        'This is a plain text version of the email. Please view in an HTML-compatible client.', // Simple fallback text
        '',
        `--${boundary}`,
        'Content-Type: text/html; charset="UTF-8"',
        'Content-Transfer-Encoding: quoted-printable', // Optimal encoding for HTML
        '',
        htmlContent,
        '',
        `--${boundary}--`,
...
export const sendEmail = async (email, code, name, type = 'register', additionalData = {}) => {
  try {
    ...
    // Select content based on email type
    // ... (Your switch statement remains exactly the same)
    switch (type) {
      case 'register':
        subject = 'Welcome to Taskit - Verify Your Email';
        htmlContent = `
    ...
    const rawMessage = createRawEmail(email, subject, finalHtmlContent);
    ...
    await gmail.users.messages.send({
        userId: 'me', // Refers to the authenticated user account
        requestBody: {
            raw: rawMessage, // The base64url encoded email body
        },
    });
...
// For backward compatibility (unchanged, still using the main sendEmail function)
export const sendVerificationEmail = async (email, verificationCode, name) => {
  return sendEmail(email, verificationCode, name, 'register');
};
