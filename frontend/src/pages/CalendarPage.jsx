...
const CalendarPage = () => {
  ...
  const filterTasks = (taskList, filterType = 'calendar', dateStr = null) => {
    return taskList.filter(task => {
        ...
        if (viewMode === 'my') {
            ...
            const isOwner = task.project.ownedBy === currentUserId;
            const isAdmin = task.project.members?.some(m => m.id === currentUserId && m.role === 'admin');
  ...
  const getStatusColor = (status) => {
     switch (status) {
         case "done": return "line-through opacity-60";
         default: return "";
     }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-6 rounded-[25px] shadow-sm border border-gray-100">
        <div className="flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-gray-500 text-sm">Manage your schedule and deadlines</p>
            </div>
        </div>
          ...
          <div className="lg:col-span-3 bg-white rounded-[25px] shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="grid grid-cols-7 auto-rows-fr bg-gray-200 gap-[1px]">
                ...
                {Array.from({ length: startingDay }).map((_, i) => (
                    <div key={`empty-${i}`} className="bg-white min-h-[140px] p-2 bg-gray-50/30" />
                ))}
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        {(provided, snapshot) => (
                            <div 
                                ...
                                className={`bg-white min-h-[140px] p-3 transition-colors ${
                                    snapshot.isDraggingOver ? 'bg-blue-50/50' : 'hover:bg-gray-50'
                                            ...
                                            {(provided) => (
                                                ...
                                                >
                                                    <Link 
                                                        ...
                                                        onClick={(e) => {
                                                            // If we are dragging, prevent navigation?
                                                            // DnD library usually handles click suppression, but good to be aware.
                                                        }}
                                                        className={`block text-xs p-2 rounded-lg border border-transparent hover:border-gray-200 hover:shadow-sm transition-all text-left group ${getPriorityColor(task.priority)} ${getStatusColor(task.status)} ${!isDraggable ? 'cursor-default' : 'cursor-grab active:cursor-grabbing'}`}
                                                    >
                                                        <div className="font-semibold truncate flex items-center gap-1">
                                                            {isDraggable && <GripVertical className="w-3 h-3 text-gray-400 opacity-0 group-hover:opacity-100" />}
                                                            {task.title}
                                                        </div>
                 ...
                 {Array.from({ length: 42 - (daysInMonth + startingDay) }).map((_, i) => (
                    <div key={`next-empty-${i}`} className="bg-white min-h-[140px] bg-gray-50/30" />
                ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
              <div className="bg-white rounded-[25px] shadow-sm border border-gray-100 p-6 h-full flex flex-col">
                  ...
                  <p className="text-sm text-gray-500 mb-4">
                      Drag tasks here to remove their due date, or drag from here to the calendar to schedule them.
                  </p>
                      ...
                      {(provided, snapshot) => (
                          <div
                              ...
                              className={`flex-1 bg-gray-50 rounded-xl p-4 space-y-3 overflow-y-auto max-h-[600px] custom-scrollbar transition-colors ${
                                  snapshot.isDraggingOver ? 'bg-blue-50 border-2 border-dashed border-blue-200' : 'border-2 border-transparent'
                              }`}
                          >
                                      ...
                                      >
                                          {(provided) => (
                                              <div
                                                  ...
                                                  {...provided.dragHandleProps}
                                                  style={{ ...provided.draggableProps.style }}
                                                  className={`bg-white p-3 rounded-xl shadow-sm border border-gray-100 group hover:shadow-md transition-all ${!isDraggable ? 'opacity-70' : 'cursor-grab active:cursor-grabbing'}`}
                                              >
                                                  <div className="flex justify-between items-start">
                                                      <h4 className="font-semibold text-gray-800 text-sm mb-1 line-clamp-2">
                                                          {task.title}
                                                      </h4>
                                                      {isDraggable && <GripVertical className="w-4 h-4 text-gray-300" />}
                                                  </div>
                                                  <div className="flex items-center gap-2 text-xs text-gray-500 mt-2">
                                                      <span className={`px-2 py-0.5 rounded-full ${getPriorityColor(task.priority)} text-[10px]`}>
                                                          {task.priority}
                                                      </span>
